...
export class CryptoLayer {
    key;
    keyVersion;
    constructor(key, keyVersion = 0) {
        this.key = key;
        this.keyVersion = keyVersion;
    }
    // ── Factory ───────────────────────────────────────────────
    static fromKey(key, keyVersion = 0) {
    ...
    static async fromPassphrase(passphrase, dataDir, keyVersion = 0) {
        ...
        const key = await new Promise((resolve, reject) => crypto.pbkdf2(passphrase, salt, PBKDF2_ITER, KEY_LEN, PBKDF2_DIGEST, (err, dk) => err ? reject(err) : resolve(dk)));
        return new CryptoLayer(key, keyVersion);
    }
    // ── Encrypt / Decrypt ─────────────────────────────────────
    /**
    ...
    reencrypt(ciphertext, newCrypto) {
        return newCrypto.encrypt(this.decrypt(ciphertext));
    }
    rotateKey(ciphertexts, newCrypto) {
        return ciphertexts.map(ct => this.reencrypt(ct, newCrypto));
    }
    static isEncryptedBuffer(buf) {
        if (buf.length < CRYPTO_OVERHEAD_V2)
            return false;
        try {
...
export class FieldCrypto {
     ...
     */
    decryptFields(doc) {
        const encryptedFields = doc[ENCRYPTED_FIELDS_KEY] ?? [];
     ...
     */
    isEncryptedField(doc, field) {
        const ef = doc[ENCRYPTED_FIELDS_KEY];
        return Array.isArray(ef) && ef.includes(field);
    }
}
export async function cryptoFromPassphrase(passphrase, dataDir, keyVersion = 0) {
    return CryptoLayer.fromPassphrase(passphrase, dataDir, keyVersion);
}
//# sourceMappingURL=crypto-layer.js.map