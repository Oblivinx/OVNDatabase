...
export class SecondaryIndexManager {
    ...
    onUpdate(oldDoc, newDoc) {
        ...
        for (const { state, newKey } of changes) {
            if (!state.def.unique || newKey === null)
                continue;
        ...
        for (const { state, oldKey, newKey } of changes) {
            if (oldKey !== null)
                this._removeFromIndex(state, oldKey, id);
            if (newKey !== null)
                this._addToIndex(state, newKey, id);
        }
    }
    onDelete(doc) {
        const id = doc['_id'];
    ...
    lookupRange(field, gte, lte) {
        ...
        for (let i = start; i < keys.length; i++) {
            const k = keys[i];
            if (lte !== undefined && k > lte)
                break;
            for (const id of state.map.get(k))
                result.push(id);
        }
        return result;
    }
    ...
    rebuildFromDocs(docs) {
        ...
        for (const doc of docs) {
            ...
            catch { /* abaikan unique violation saat rebuild */ }
        }
    ...
    _removeFromIndex(state, key, id) {
        ...
        if (set.size === 0) {
            ...
            if (pos >= 0)
                state.sortedKeys.splice(pos, 1);
        }
        state.dirty = true;
    }
    _docMatchesPartial(doc, state) {
        if (!state.def.partial)
            return true;
        return matchFilter(doc, state.def.partial);
    }
    _getField(doc, fieldPath) {
        return fieldPath.split('.').reduce((o, k) => o !== null && typeof o === 'object' ? o[k] : undefined, doc);
    }
    _idxPath(field) {
...
//# sourceMappingURL=secondary-index.js.map