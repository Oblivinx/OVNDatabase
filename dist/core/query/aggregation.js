...
// ============================================================
import { matchFilter, getFieldValue, setNestedField } from './filter.js';
import { validateCollectionName, isDangerousKey } from '../../utils/security.js';
/**
 ...
 */
export function compilePipeline(pipeline, lookupResolver) {
    return async (docs) => {
        let result = docs;
...
async function applyStage(stage, docs, lookupResolver) {
    if ('$match' in stage) {
        return docs.filter(d => matchFilter(d, stage.$match));
    }
    if ('$project' in stage) {
        return docs.map(d => applyProjectStage(d, stage.$project));
    }
    if ('$addFields' in stage) {
        return docs.map(d => applyAddFields(d, stage.$addFields));
    }
    if ('$group' in stage) {
    ...
    if ('$unwind' in stage) {
        return applyUnwind(docs, stage.$unwind);
    }
    if ('$count' in stage) {
...
function applyProjectStage(doc, spec) {
    ...
    for (const [field, value] of Object.entries(spec)) {
        ...
        if (value === 1) {
            ...
            result[field] = evaluateExpression(value, doc);
        }
        else {
            result[field] = value;
        }
...
function applyAddFields(doc, spec) {
    ...
    for (const [field, value] of Object.entries(spec)) {
        if (typeof value === 'string' && value.startsWith('$')) {
            ...
            setNestedField(result, field, evaluateExpression(value, doc));
        }
        else {
...
function applyGroup(docs, spec) {
    ...
    return [...groups.values()].map(({ key, docs: grpDocs }) => {
        ...
        for (const [field, expr] of Object.entries(spec)) {
            if (field === '_id')
                continue;
            out[field] = evaluateAccumulator(expr, grpDocs);
        }
        return out;
    });
}
function evaluateAccumulator(expr, docs) {
    if ('$sum' in expr) {
        const field = expr.$sum;
        if (typeof field === 'number')
            return field * docs.length; // $sum: 1 → count
    ...
    if ('$avg' in expr) {
        const path = (expr.$avg ?? '').slice(1);
        const nums = docs.map(d => getFieldValue(d, path)).filter(v => typeof v === 'number');
        return nums.length ? nums.reduce((s, v) => s + v, 0) / nums.length : null;
    }
    if ('$min' in expr) {
        const path = (expr.$min ?? '').slice(1);
        const vals = docs.map(d => getFieldValue(d, path)).filter(v => v !== undefined);
        return vals.length ? Math.min(...vals) : null;
    }
    if ('$max' in expr) {
        const path = (expr.$max ?? '').slice(1);
        const vals = docs.map(d => getFieldValue(d, path)).filter(v => v !== undefined);
...
// ── $unwind ──────────────────────────────────────────────────
function applyUnwind(docs, spec) {
    ...
    for (const doc of docs) {
        ...
        if (!Array.isArray(val) || val.length === 0) {
            if (preserve)
                result.push({ ...doc, [field]: val ?? null });
            continue;
        }
...
// ── $lookup ──────────────────────────────────────────────────
async function applyLookup(docs, spec, resolver) {
    ...
    for (const fd of foreignDocs) {
        ...
        if (!index.has(serialized))
            index.set(serialized, []);
...
function evaluateExpression(expr, doc) {
    if ('$concat' in expr) {
        return expr.$concat.map(part => typeof part === 'string' && part.startsWith('$')
            ? String(getFieldValue(doc, part.slice(1)) ?? '')
            : String(part)).join('');
    }
    if ('$add' in expr) {
        return expr.$add.reduce((sum, part) => {
    ...
    if ('$multiply' in expr) {
        return expr.$multiply.reduce((prod, part) => {
            const v = typeof part === 'string' && part.startsWith('$')
                ? getFieldValue(doc, part.slice(1)) : part;
    ...
    if ('$cond' in expr) {
        ...
        const test = matchFilter(doc, cond.if);
...
function applyBucket(docs, spec) {
    ...
    if (def !== undefined)
        buckets.set('__default__', { _id: def, count: 0, docs: [] });
    for (const doc of docs) {
    ...
    return [...buckets.values()].map(({ _id, count, docs: grpDocs }) => {
        const result = { _id, count };
        if (output) {
            for (const [field, expr] of Object.entries(output)) {
                result[field] = evaluateAccumulator(expr, grpDocs);
            }
        }
        return result;
    }).filter(b => b['count'] > 0 || b['_id'] === def);
}
// ── G11: $facet ──────────────────────────────────────────────
async function applyFacet(docs, facets, lookupResolver) {
    const result = {};
    // Jalankan semua sub-pipeline secara paralel
    await Promise.all(Object.entries(facets).map(async ([name, pipeline]) => {
...
// ── G11: $densify ─────────────────────────────────────────────
function applyDensify(docs, spec) {
    const { field, range } = spec;
    if (range.bounds === 'full' || range.bounds === 'partition')
...
// ── G11: $setWindowFields ─────────────────────────────────────
function applySetWindowFields(docs, spec) {
        ...
        for (const [outField, expr] of Object.entries(output)) {
            ...
            if ('$sum' in expr) {
                const path = typeof expr.$sum === 'string' && expr.$sum.startsWith('$')
                    ? expr.$sum.slice(1) : null;
                result[outField] = path
                    ? windowDocs.reduce((s, d) => s + (getFieldValue(d, path) || 0), 0)
                    : expr.$sum * windowDocs.length;
            }
            else if ('$avg' in expr) {
                const path = typeof expr.$avg === 'string' && expr.$avg.startsWith('$')
                    ? expr.$avg.slice(1) : null;
                if (path) {
                    const nums = windowDocs.map(d => getFieldValue(d, path)).filter(v => typeof v === 'number');
                ...
                result[outField] = sorted.filter(d => getFieldValue(d, sortField) < val).length + 1;
            }
            else if ('$denseRank' in expr) {
                const sortField = Object.keys(sortBy)[0];
                const val = getFieldValue(doc, sortField);
                const uniqueSmaller = new Set(sorted
                    .filter(d => getFieldValue(d, sortField) < val)
...
function getWindowDocs(sorted, currentIdx, window) {
    const [start, end] = window;
    const lo = start === 'unbounded' ? 0 : currentIdx + start;
    const hi = end === 'current' ? currentIdx : end === 'unbounded' ? sorted.length - 1 : currentIdx + end;
