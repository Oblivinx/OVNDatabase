...
export class MVCCManager {
    ...
    commit(txId) {
        ...
        if (!snapshot)
            throw new Error(`[MVCC] Unknown transaction: ${txId}`);
        const intents = this._writeIntents.get(txId) ?? new Set();
        ...
        for (const otherTxId of this._committed) {
            if (otherTxId === txId)
                continue; // jangan cek diri sendiri
            ...
            if (snapshot.visibleTxIds.has(otherTxId))
                continue;
            ...
            if (otherIntents) {
                for (const key of intents) {
                    if (otherIntents.has(key)) {
                        ...
                        throw new WriteConflictError(`[MVCC] Write conflict on key "${key}" between tx ${txId} and ${otherTxId}`);
                    }
                }
            }
        }
     ...
     */
    isVisible(recordTxId, snap) {
        if (recordTxId === snap.txId)
            return true; // bisa lihat write sendiri
    ...
    get nextTxId() { return this._nextTxId; }
    // ── Garbage Collection ────────────────────────────────────
    /**
     ...
     */
    _maybeGCCommitted() {
        if (this._committed.size < 1000)
            return;
        let oldestActive = this._nextTxId;
        for (const snap of this._active.values()) {
            if (snap.txId < oldestActive)
                oldestActive = snap.txId;
        }
        ...
        if (gcCount > 0)
            log.debug(`MVCC GC: removed ${gcCount} old txIds`);
    }
}
