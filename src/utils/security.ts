...
import type { QueryFilter } from '../types/index.js';
 ...
 */
export function validateFieldPath(fieldPath: string): void {
...
export function validateCollectionName(name: string): void {
  ...
  if (!COLLECTION_NAME_RE.test(name))
    throw new Error(
      `[OvnDB] Nama collection tidak valid: hanya huruf, angka, _ dan - ` +
 ...
 */
export function assertPathInside(base: string, target: string, label = 'path'): void {
 ...
 */
export function validateDocumentSize(buf: Buffer): void {
  if (buf.length > MAX_DOCUMENT_BYTES) {
    throw new Error(
      `[OvnDB] Dokumen melebihi ukuran maksimum (${buf.length} > ${MAX_DOCUMENT_BYTES} bytes)`
    );
  }
}
 ...
 */
export function validateQueryFilter(filter: unknown, depth = 0): void {
  ...
  if (Array.isArray(filter)) {
    for (const item of filter) validateQueryFilter(item, depth + 1);
    return;
  }
  ...
  const keys = Reflect.ownKeys(obj).filter(k => typeof k === 'string') as string[];
  ...
  for (const key of keys) {
    ...
    const val = (obj as unknown as { [key: string]: unknown })[key];
    ...
    if ((key === '$in' || key === '$nin') && Array.isArray(val)) {
      if (val.length > MAX_IN_ARRAY_SIZE)
        throw new Error(
          `[OvnDB] ${key} terlalu banyak elemen (${val.length} > maks ${MAX_IN_ARRAY_SIZE})`
        );
    }
    ...
    if (key === '$and' || key === '$or' || key === '$nor') {
      ...
      for (const sub of val) validateQueryFilter(sub, depth + 1);
 ...
 */
export function validateRegex(pattern: unknown): void {
  if (pattern instanceof RegExp) return; // sudah dikompilasi, aman
  if (typeof pattern !== 'string')
    throw new Error('[OvnDB] $regex harus berupa string atau RegExp');
  if (pattern.length > MAX_REGEX_PATTERN_LEN)
    throw new Error(
      `[OvnDB] $regex pattern terlalu panjang (${pattern.length} > maks ${MAX_REGEX_PATTERN_LEN})`
    );
 ...
 */
export function validateUpdateSpec(spec: Record<string, unknown>, depth = 0): void {
  if (depth > MAX_QUERY_DEPTH)
  ...
  for (const op of Object.keys(spec)) {
    ...
    const opProto = Object.getPrototypeOf(opVal as object);
    ...
    // FIX: use Reflect.ownKeys to catch __proto__ which Object.keys() misses
    const fieldKeys = Reflect.ownKeys(opVal as object)
    ...
    if (op === '$push') {
      for (const [, v] of Object.entries(opVal as Record<string, unknown>)) {
        if (v !== null && typeof v === 'object' && '$each' in (v as object)) {
          const each = (v as Record<string, unknown>)['$each'];
          if (Array.isArray(each) && each.length > MAX_IN_ARRAY_SIZE)
            throw new Error(
              `[OvnDB] $push.$each terlalu banyak elemen (maks ${MAX_IN_ARRAY_SIZE})`
            );
        }
      }
    }
 ...
 */
export function validateDocumentKeys(doc: unknown, depth = 0): void {
  if (depth > MAX_QUERY_DEPTH) return; // terlalu dalam — size limit menangani ini
  if (doc === null || typeof doc !== 'object') return;
  if (Array.isArray(doc)) {
    for (const item of doc) validateDocumentKeys(item, depth + 1);
    return;
  }
  ...
  // FIX: use Reflect.ownKeys to catch __proto__ which Object.keys() misses
  for (const key of Reflect.ownKeys(doc as Record<string, unknown>)) {
    if (typeof key !== 'string') continue;
    if (isDangerousKey(key))
      throw new Error(`[OvnDB] Dokumen mengandung kunci terlarang: "${key}"`);
    validateDocumentKeys((doc as Record<string, unknown>)[key], depth + 1);
  }
}
 ...
 */
export function validateDocumentId(id: unknown): void {
  if (typeof id !== 'string')
