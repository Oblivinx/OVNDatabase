 ...
 */
export async function exportTo<T extends OvnDocument>(
  col: Collection<T>,
  filePath: string,
  opts: ExportOptions = {},
): Promise<number> {
  ...
  if (format === 'ndjson') {
    ...
    const ws = fd.createWriteStream({ encoding: 'utf8' }) as unknown as Writable;
    try {
      const docs = await col.find({});
      for (const doc of docs) {
        const out = opts.projection ? applyProjection(doc as Record<string, unknown>, opts.projection) : doc;
    ...
    const out  = opts.projection
      ? docs.map(d => applyProjection(d as Record<string, unknown>, opts.projection!))
      : docs;
 ...
 */
export async function importFrom<T extends OvnDocument>(
  col: Collection<T>,
  filePath: string,
  opts: ImportOptions = {},
...
async function importNdjson<T extends OvnDocument>(
  col: Collection<T>,
  filePath: string,
  upsert: boolean,
  ...
  for await (const line of rl) {
    ...
    lineNum++;
    try {
      const doc = JSON.parse(trimmed) as T;
...
async function importJson<T extends OvnDocument>(
  col: Collection<T>,
  filePath: string,
  upsert: boolean,
  continueOnError: boolean,
...
async function insertOrUpsert<T extends OvnDocument>(
  col: Collection<T>,
  doc: T,
  upsert: boolean,
  result: ImportResult,
  ...
  if (upsert && doc._id) {
    await col.upsertOne({ _id: doc._id }, { $set: doc as Record<string, unknown> });
    result.inserted++;
  } else {
    await col.insertOne(doc as Omit<T, '_id'> & { _id?: string });
    result.inserted++;
  }
}
